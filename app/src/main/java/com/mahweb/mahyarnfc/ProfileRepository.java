...
public final class ProfileRepository {
    ...
    private static final String K_ONBOARDING = "onboarding_completed";
    private static final String K_PROFILE_SCHEMA = "profile_schema_version";
    ...
    public static void save(Context c, Profile r) {
        prefs(c).edit()
                ...
                .putInt(K_PROFILE_SCHEMA, 2)
                .apply();
    }

    public static boolean isOnboardingCompleted(Context c) {
        return prefs(c).getBoolean(K_ONBOARDING, false);
    }

    public static void setOnboardingCompleted(Context c, boolean completed) {
        prefs(c).edit()
                .putBoolean(K_ONBOARDING, completed)
                .putInt(K_PROFILE_SCHEMA, 2)
                .apply();
    }

    public static boolean hasUsableProfile(Context c) {
        return ProfileValidator.isProfileReady(load(c));
    }

    public static int profileSchemaVersion(Context c) {
        return prefs(c).getInt(K_PROFILE_SCHEMA, 1);
    }
