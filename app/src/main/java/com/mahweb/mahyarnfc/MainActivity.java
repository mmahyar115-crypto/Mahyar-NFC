...
public class MainActivity extends Activity implements NfcReader.Listener, NfcShareEvents.Listener {
    ...
    private HceSessionController hceSessionController;
    private TextView sendReaderStatus;
    private TextView sendUniversalStatus;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        hceSessionController = new HceSessionController(this);
    ...
    private void showDashboard() {
        ...
        view.findViewById(R.id.btnNearbyAnalyzer).setOnClickListener(v -> startActivity(new Intent(this, NearbyActivity.class)));
    ...
    private void showSendScreen() {
        ...
        setNavActive(SCREEN_HOME);
        headerSubtitle.setText("ارسال Universal");
        ...
        sendReaderStatus = view.findViewById(R.id.sendReaderStatus);
        sendUniversalStatus = view.findViewById(R.id.sendUniversalStatus);
        ...
        Button testUniversalButton = view.findViewById(R.id.btnTestUniversalCard);
        ...
        boolean universalSupported = UniversalCardPayload.isUniversalPayloadSupported(profile, UniversalCardPayload.DEFAULT_BRIDGE_BASE_URL);
        ...
        sendReaderStatus.setVisibility(View.GONE);
        ProfileRepository.setShareEnabled(this, false);

        if (universalSupported) {
            sendUniversalStatus.setText("سازگاری Universal آماده است — Android با اپ Mahyar مستقیم؛ iPhone XS یا جدیدتر و Readerهای استاندارد از NDEF/URL استفاده می‌کنند.");
            sendUniversalStatus.setTextColor(getColor(R.color.success));
        } else {
            sendUniversalStatus.setText("اطلاعات این پروفایل برای بسته Universal بیش از حد بزرگ است. Android مستقیم و QR همچنان قابل استفاده‌اند.");
            sendUniversalStatus.setTextColor(getColor(R.color.danger));
        }
        ...
        btnToggleShare.setOnClickListener(v -> {
            ...
            if (active) {
                ProfileRepository.setShareEnabled(this, true);
                boolean preferred = hceSessionController.activate(this);
                if (!preferred) {
                    ProfileRepository.setShareEnabled(this, false);
                    statusTitle.setText("NFC در اختیار Mahyar قرار نگرفت");
                    statusTitle.setTextColor(getColor(R.color.danger));
                    statusDescription.setText("سیستم نتوانست سرویس HCE را در اولویت قرار دهد. NFC را یک‌بار خاموش/روشن کنید و دوباره تلاش کنید؛ QR همچنان آماده است.");
                    instruction.setVisibility(View.GONE);
                    sendReaderStatus.setVisibility(View.GONE);
                    return;
                }
                NfcShareEvents.setListener(this);
                getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
                statusTitle.setText("آماده ارسال Universal");
                statusTitle.setTextColor(getColor(R.color.success));
                statusDescription.setText("Android مقابل: تب «دریافت» Mahyar را باز کند. iPhone: صفحه روشن باشد و بالای گوشی را به پشت این Android نزدیک کند، سپس اعلان NFC را لمس کند.");
                instruction.setText("آماده — گوشی مقابل را نزدیک محل آنتن NFC نگه دارید");
                instruction.setVisibility(View.VISIBLE);
                sendReaderStatus.setText("منتظر شناسایی گوشی مقابل…");
                sendReaderStatus.setVisibility(View.VISIBLE);
                btnToggleShare.setText("توقف ارسال Universal");
            } else {
                ProfileRepository.setShareEnabled(this, false);
                NfcShareEvents.clearListener(this);
                hceSessionController.deactivate(this);
                getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
                statusTitle.setText(NfcState.title(status));
                statusDescription.setText(NfcState.description(status));
                instruction.setVisibility(View.GONE);
                sendReaderStatus.setVisibility(View.GONE);
                btnToggleShare.setText("فعال کردن ارسال Universal");
            }
        });
        ...
        testUniversalButton.setEnabled(universalSupported);
        testUniversalButton.setAlpha(universalSupported ? 1f : 0.45f);
        testUniversalButton.setOnClickListener(v -> {
            try {
                String url = UniversalCardPayload.bridgeUrl(ProfileRepository.load(this), UniversalCardPayload.DEFAULT_BRIDGE_BASE_URL);
                startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(url)));
            } catch (RuntimeException e) {
                Toast.makeText(this, "ساخت لینک Universal ناموفق بود", Toast.LENGTH_SHORT).show();
            }
        });
        replaceContent(view);
    }
    ...
    private void leaveActiveNfcModes() {
        ...
        ProfileRepository.setShareEnabled(this, false);
        NfcShareEvents.clearListener(this);
        if (hceSessionController != null) hceSessionController.deactivate(this);
        getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
    }
    ...
    @Override public void onReaderDetected(NfcShareEvents.Protocol protocol) {
        runOnUiThread(() -> {
            if (currentScreen != SCREEN_SEND || sendReaderStatus == null || !ProfileRepository.isShareEnabled(this)) return;
            if (protocol == NfcShareEvents.Protocol.MAHYAR_DIRECT) {
                sendReaderStatus.setText("Android با Mahyar شناسایی شد — در حال ارسال مستقیم…");
            } else {
                sendReaderStatus.setText("Reader استاندارد / iPhone شناسایی شد — در حال ارائه کارت NDEF…");
            }
            sendReaderStatus.setTextColor(getColor(R.color.primary));
            sendReaderStatus.setVisibility(View.VISIBLE);
        });
    }

    @Override public void onTransferComplete(NfcShareEvents.Protocol protocol) {
        runOnUiThread(() -> {
            if (currentScreen != SCREEN_SEND || sendReaderStatus == null) return;
            sendReaderStatus.setText(protocol == NfcShareEvents.Protocol.MAHYAR_DIRECT
                    ? "ارسال مستقیم Android با موفقیت کامل شد"
                    : "کارت Universal خوانده شد");
            sendReaderStatus.setTextColor(getColor(R.color.success));
            sendReaderStatus.setVisibility(View.VISIBLE);
        });
    }

    @Override public void onFieldLost() {
        runOnUiThread(() -> {
            if (currentScreen != SCREEN_SEND || sendReaderStatus == null || !ProfileRepository.isShareEnabled(this)) return;
            sendReaderStatus.setText("آماده ارسال مجدد — گوشی بعدی را نزدیک کنید");
            sendReaderStatus.setTextColor(getColor(R.color.text_secondary));
            sendReaderStatus.setVisibility(View.VISIBLE);
        });
    }
